...
import static tester.hr.ScoreServiceFactory.createConcurrentScoreService;
...
abstract public class Validator {
	...
	static private IScoreService resolveImplementation() {
		return createConcurrentScoreService();
	}
	...
	static private final String[] NAMES = new String[] { "Orval", "Georgiana", "Paul", "Merlene", "Nicola", "Landon",
			"Nobuko", "Tory", "Leeanne", "Elwanda", "Lashonda", "William", "Belle", "Rosaura", "Isaura", "Jesenia",
			"Wes", "Shawanda", "Jill", "Levi", "Dennise", "Yolando", "Jacinda", "Masako", "Keli", "Madalene",
			"Mohammed", "Dalia", "Hung", "Carola", "Graig", "Emmaline", "Jimmy", "Phylicia", "Nathanael", "Joanie",
			"Shala", "Christie", "Alverta", "Titus", "Maryetta", "Melia", "Isabell", "Rosie", "Earline", "Kristopher",
			"Sherwood", "Alvin", "Dominga", "Emiko" };

	static private final String BIG_HASH = ""
	...
	static private ScoreOperation[] createOperations() {
		...
		for (int i = 0; i < operations.length; i++) {
			...
			try {
				...
				if (bs.length < 8)
					throw new RuntimeException("length=" + bs.length);
				g = ByteBuffer.wrap(bs).getLong();
			} catch (RuntimeException e) {
				System.out.println("g=" + g + " str=" + str);
				if (bs != null)
					System.out.println("bs.length=" + bs.length + " " + Arrays.toString(bs));
				throw e;
			}
			g = Math.abs(g);
...
//            System.out.println(s.ticker() + " " + pointsMap.get(s.getUserId()));
	...
	static private void checkRanking(List<IScore> iScores) {
		...
		for (IScore score : iScores) {
			...
			if (currPos < prevPos)
				throw new RuntimeException("Ranking position error");
			if (currScore > prevScore)
				throw new RuntimeException("Ranking score error");
			if (currPos == prevPos && currScore != prevScore)
				throw new RuntimeException("Ranking tie error");
